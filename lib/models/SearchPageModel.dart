...
class SearchPageModel {
    ...
    Future<List<SongCardInPlaylistModel>> querySongByName(String songName) async {
        Uri httpURI = Uri(scheme: 'http', host: SV_HOSTNAME, path: SEARCH_SONG_PATH, port: SV_PORT, queryParameters: {"song_name": songName});
    ...
    Future<List<ArtistRectangleCardModel>> queryArtistByName(String artistName) async {
        Uri httpURI = Uri(scheme: 'http', host: SV_HOSTNAME, path: SEARCH_ARTIST_PATH, port: SV_PORT, queryParameters: {"artist_name": artistName, });
        dynamic res = await http.get(httpURI);
    ...
    Future<List<PlaylistRectangleCardModel>> queryPlaylistByName(String playlist_name) async {
        Uri httpURI = Uri(scheme: 'http', host: SV_HOSTNAME, path: SEARCH_PLAYLIST_PATH, port: SV_PORT, queryParameters: {"playlist_name": playlist_name, "app_token": GetIt.I.get<CredentialModelNotifier>().value.appToken, "public": "1"});
        dynamic res = await http.get(httpURI);
    ...
    Future<List<AlbumRectangleCardModel>> queryAlbumByName(String albumName) async {
        Uri httpURI = Uri(scheme: 'http', host: SV_HOSTNAME, path: SEARCH_ALBUM_PATH, port: SV_PORT, queryParameters: {"album_name": albumName});
        dynamic res = await http.get(httpURI);
...
class SearchPageModelNotifier extends ValueNotifier<SearchPageModel> {
    SearchPageModelNotifier(SearchPageModel value) : super(value);
    ...
    void changeSearchString(String searchString) {
        ...
        if (value.searchString != searchString) {
          ...
          switch (value.searchMode){
          case "song_name" : 
            value.queryResult = value.querySongByName(searchString);
            notifyListeners();
            break;
          case "artist_name" :
            value.queryResult = value.queryArtistByName(searchString);
            notifyListeners();
            break;
          
          case "album_name" :
            value.queryResult = value.queryAlbumByName(searchString);
            notifyListeners();
            break;

          case "playlist_name" :
            value.queryResult = value.queryPlaylistByName(searchString);
            notifyListeners();
          }
        } 
