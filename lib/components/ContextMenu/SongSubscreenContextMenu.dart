...
class SongSubscreenContextMenu extends SubscreenContextMenu {
  SongSubscreenContextMenu({this.songCardInPlaylistModel}): super(
    body: (context) {
        ...
        padding: const EdgeInsets.symmetric(horizontal: kDefaultPadding),
          ...
          child: Container(
            ...
            child: MaterialApp(
              ...
              home: ViewAllPlaylistsPage(songToAdd: songCardInPlaylistModel)
            ),
          ),
      );
    },
...
Route _createRoutePageViewSongPlaylist(PlaylistRectangleCardModel model, SongCardInPlaylistModel? songToAdd) {
  return PageRouteBuilder(
    pageBuilder: (context, animation, secondaryAnimation) => ViewAllSongsInPlaylist(unPopulatedModel: model, songToAdd: songToAdd),
    transitionsBuilder: (context, animation, secondaryAnimation, child) {
      const begin = Offset(1.0, 0.0);
      const end = Offset.zero;
      const curve = Curves.ease;
...
class ViewAllPlaylistsPage extends StatelessWidget {
  ViewAllPlaylistsPage({Key? key, this.songToAdd}) : super(key: key);
            ...
            Container(
              ...
              padding: const EdgeInsets.symmetric(horizontal: kDefaultPadding),
                margin: const EdgeInsets.only(bottom: kDefaultPadding),
                  child: const Text('Chọn Playlist', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16), ),
            ),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: kDefaultPadding),
                      ...
                      if (Navigator.of(context).canPop())
                        ...
                        CustomTextButton.TextButton(
                          ...
                          onTap: () {
                            ...
                            GetIt.I.get<UserModelNotifier>().addPlaylist(titleController.text, descriptionController.text, image!.path);
                            // Navigator.of(context).push(_createRoutePageCreatePlaylist());
                          }
                        ),
                    ], ),
                            ...
                            child: GestureDetector(
                              ...
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  borderRadius: BorderRadius.circular(10),
                                  image: image != null ? DecorationImage(image: Image.asset(image!.path).image, fit: BoxFit.cover) : null
                                ),
...
class ViewAllSongsInPlaylist extends StatefulWidget {
  ViewAllSongsInPlaylist({Key? key, required this.unPopulatedModel, this.songToAdd}):super(key: key);
                  ...
                  if (widget.songToAdd != null)
                    CustomTextButton.TextButton(
                      ...
                      onTap: () async {
                        ...
                        bool success = await widget.unPopulatedModel.addSong(widget.songToAdd!.id);
                        if (success) {
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (snapshot.hasData && snapshot.connectionState == ConnectionState.done) {
                      ...
                      for (Map < String, dynamic > json in model.songs) {
                        (json['song'] as Map<String, dynamic>).addAll({'artist' : json['artist'], 'album': json['album']});
                        songsWidget.add(
